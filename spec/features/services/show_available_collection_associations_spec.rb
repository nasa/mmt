...
describe 'Show Available Service Collection Associations', js: true, reset_provider: true do
  ...
  context 'when there are paginated collection associations' do
    before do
      ...
      provider_collections_page_1_response = Cmr::Response.new(Faraday::Response.new(status: 200, body: JSON.parse(File.read('spec/fixtures/services/provider_collections.json'))))
      provider_options = {page_size: 500, provider: 'MMT_2', entry_title: '*', options: {entry_title: {pattern: true, ignore_case: true}}}
      allow_any_instance_of(Cmr::CmrClient).to receive(:get_collections_by_post).with(provider_options, 'access_token').and_return(provider_collections_page_1_response)

      # mock for all collections associated to this service
      collection_associations_page_response = Cmr::Response.new(Faraday::Response.new(status: 200, body: JSON.parse(File.read('spec/fixtures/services/service_associated_collections.json'))))
      collection_associations_page_options = {page_size: 2000, page_num: 1, service_concept_id: @service_ingest_response['concept-id']}
      allow_any_instance_of(Cmr::CmrClient).to receive(:get_collections_by_post).with(collection_associations_page_options, 'access_token').and_return(collection_associations_page_response)
