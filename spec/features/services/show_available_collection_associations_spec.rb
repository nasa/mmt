...
describe 'Show Available Service Collection Associations', js: true, reset_provider: true do
  ...
  context 'when there are paginated collection associations' do
    before do
      ...
      collection_associations_response1 = Cmr::Response.new(Faraday::Response.new(status: 200, body: JSON.parse(File.read('spec/fixtures/services/service_associated_collections.json'))))
      options_2 = {page_size: 2000, page_num: 1, service_concept_id: @service_ingest_response['concept-id']}
      allow_any_instance_of(Cmr::CmrClient).to receive(:get_collections_by_post).with(options_2, 'access_token').and_return(collection_associations_response1)
    ...
    #     page.save_screenshot('/tmp/step3.png')
    #   end
    # end
