describe 'Publishing variable draft records' do
  ...
  context 'when publishing a variable draft record' do
    before do
      login
      draft = create(:full_variable_draft, user: User.where(urs_uid: 'testuser').first, draft_short_name: '12345', draft_entry_title: 'Draft Title', collection_concept_id: @collection_ingest_response['concept-id'])
