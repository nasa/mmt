  ...
  context 'when the form is complete' do
    ...
    context 'when there is an error communicating with CMR' do
      before do
        ...
        allow_any_instance_of(Cmr::CmrClient).to receive(:check_user_permissions).with({ concept_id: @ingest_response['concept-id'], user_id: 'rarxd5taqea' }, 'access_token').and_return(cmr_response)
        click_on 'Estimate Notification Frequency'
      end
