...
class CollectionDraftsController < BaseDraftsController
  ...
  def validate_metadata
    ...
    errors = Array.wrap(JSON::Validator.fully_validate(@json_schema, fix_metadata_fields(get_resource.draft)))
  ...
  def fix_metadata_fields(meta)
    if meta.dig('UseConstraints','LicenseURL')
      meta['UseConstraints']['LicenseUrl']  = meta['UseConstraints']['LicenseURL']
      meta['UseConstraints'].delete('LicenseURL')
    end
    meta
  end
