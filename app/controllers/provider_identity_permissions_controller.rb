class ProviderIdentityPermissionsController < ManageCmrController
  ...
  def update
    ...
    unless fails.blank? && overwrite_fails.blank?
      ...
      overwrite_error_message = overwrite_fails.join(', ').titleize + ' permissions were unable to be saved because another user made changes to those permissions.' unless error_message2.present?
