class Draft < ActiveRecord::Base
  ...
  def get_preview_spatial(draft)
    ...
    if draft.draft['SpatialExtent'] &&
      ...
      if !geometry['BoundingRectangles'].blank?
        ...
        geometry['BoundingRectangles'].each do |bounding_rectangle|
          ...
          x1, y1 = convert_lat_lon_to_image_x_y(nbc, wbc, map_width, map_height)
          x2, y2 = convert_lat_lon_to_image_x_y(sbc, ebc, map_width, map_height)
          min_x = [x1, x2].min
          max_x = [x1, x2].max
          min_y = [y1, y2].min
          max_y = [y1, y2].max
    ...
    return point_coordinate_array, rectangle_coordinate_array
  end


  def get_preview_temporal(draft)
    ...
    return single_date_times, range_date_times, periodic_date_times
  end
