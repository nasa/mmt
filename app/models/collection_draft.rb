class CollectionDraft < Draft
  ...
  # By the final PR for master, this function should be removed, but it exists to verify that the migration will work as expected
  def migration_test
    return self unless draft['SpatialInformation'].present? && ['HORIZONTAL', 'BOTH'].include?(draft['SpatialInformation']['SpatialCoverageType'])
    ...
    draft.delete('SpatialInformation')
    draft.delete('SpatialExtent')

    draft['SpatialInformation'] = spatial_information if spatial_information
    draft['SpatialExtent'] = spatial_extent if spatial_extent
    self
  end
