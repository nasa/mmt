...
describe('Layout Grid Field Component', () => {
  ...
  test('testing custom checkbox field', async () => {
    const model = new UmmServicesModel()
    const editor = new MetadataEditor(model)
    const { container } = render(
      <MemoryRouter initialEntries={['/service_drafts/1/edit/Options']}>
        <Routes>
          <Route path="/service_drafts/:id/edit/:sectionName" element={<MetadataEditorForm editor={editor} />} />
        </Routes>
      </MemoryRouter>
    )
    const spatialCheckbox = screen.queryByTestId('layoutgridfield--spatial__checkbox')

    fireEvent.click(await spatialCheckbox)

    const pointCheckbox = screen.queryByTestId('layoutgridfield--point__checkbox')
    fireEvent.click(await pointCheckbox)

    const circleCheckbox = screen.queryByTestId('layoutgridfield--circle__checkbox')
    fireEvent.click(await circleCheckbox)

    const pointSection = within(screen.queryByTestId('layout-grid-field__schema-field--point'))
    const pointRadioButton = pointSection.queryByTestId('custom-radio-widget--value__true')
    fireEvent.click(await pointRadioButton)

    const circleSection = within(screen.queryByTestId('layout-grid-field__schema-field--circle'))
    const circleRadioButton = circleSection.queryByTestId('custom-radio-widget--value__true')
    fireEvent.click(await circleRadioButton)

    const { draft: beforeDraft } = removeEmpty(editor.draft)
    fireEvent.click(await pointCheckbox)
    const { draft: afterDraft } = removeEmpty(editor.draft)

    expect(beforeDraft).toEqual({ ServiceOptions: { Subset: { SpatialSubset: { Point: { AllowMultipleValues: true }, Circle: { AllowMultipleValues: true } } } } })
    expect(afterDraft).toEqual({ ServiceOptions: { Subset: { SpatialSubset: { Circle: { AllowMultipleValues: true } } } } })

    fireEvent.click(await spatialCheckbox)
    const { draft: endDraft } = removeEmpty(editor.draft)
    expect(endDraft).toEqual(undefined)
