...
function traverse(node: any, parent: string, name: string, path: string[], paths: any[], filter: string[]) {
  ...
  children.forEach((child: any) => {
    ...
    if (child.subfields) {
      child.subfields.forEach((subfield: any) => {
...
function traverseArrays(parent: any, tokens: string[]) {
  if (tokens.length > 0) {
...
export function parseCmrResponse(response: any, filter: string[]) {
  let paths = [] as string[][]
  const path = [] as any[]
