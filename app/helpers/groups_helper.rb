...
module GroupsHelper
  ...
  def group_provider(group)
    check_if_system_group?(group, group['group_id']) ? 'CMR' : group['provider_id']
  end
  ...
  def render_manage_provider_or_system_permissions_title
    if @user_is_system_acl_admin && check_if_system_group?(@group, @group_id)
  ...
  def render_manage_system_permissions_link
    concat(content_tag(:p) do
      concat(link_to('System Object Permissions', edit_system_identity_permission_path(@group_id, redirect_to: request.fullpath)))
    end)
  end
