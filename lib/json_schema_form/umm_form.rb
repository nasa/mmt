...
class UmmForm < JsonObj
  ...
  def initialize(form_section_json, json_form, schema, options = {})
    ...
    @description = @schema.retrieve_schema_fragment(@parsed_json['key']).fetch('description', @parsed_json['description'])
...
class UmmFormElement < UmmForm
  ...
  def render_markup
    capture do
      ...
      concat help_icon(help_path, form_fragment['key'].split('/').last.titleize)
